    ...
    public function up(): void
    {
        ...
        */
        ...
        if ($driver === 'sqlite') {
            ...
            DB::statement("
                CREATE VIEW client_bond_summary AS
                SELECT
                    ...
                    COUNT(DISTINCT CASE
                        WHEN cbr.type = 'renewal'
                            AND cbr.approved = 1
                            AND strftime('%m', cbr.created_at) = strftime('%m', 'now')
                            AND strftime('%Y', cbr.created_at) = strftime('%Y', 'now')
                        THEN cbr.id
                        ...
                        FROM (
                            SELECT cb2.id AS bond_id,
                                   ...
                                   (
                                       ...
                                       FROM client_bond_payouts cbp
                                       WHERE cbp.client_bond_id = cb2.id
                                         AND strftime('%m', cbp.created_at) = strftime('%m', 'now')
                                         AND strftime('%Y', cbp.created_at)  = strftime('%Y', 'now')
                            ...
                            WHERE EXISTS (
                                ...
                                WHERE cbr2.client_bond_id = cb2.id
                                  AND cbr2.type           = 'liquidation'
                                  AND cbr2.approved       = 1
                                  AND strftime('%m', cbr2.created_at) = strftime('%m', 'now')
                                  AND strftime('%Y', cbr2.created_at)  = strftime('%Y', 'now')
                            )
                        ) liq
